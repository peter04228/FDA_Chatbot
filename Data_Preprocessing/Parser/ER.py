...
from annex_common import attach_annex_references_to_tree


# --------------------------------------------------
# 경로 설정
# --------------------------------------------------
INPUT_DIR = r"C:\dev\FDA_Chatbot\DATA\o_data\시행규칙\본문"
OUT_DIR = r"C:\dev\FDA_Chatbot\DATA\parser_data\ER"
...
# ER 문서 메타 생성
# --------------------------------------------------
def build_er_metadata(file_name, source_path):
    return {
        "document_group": None,
...
def process_one_pdf(pdf_path):
    ...
    # ---------- 별표 reference 부착 ----------
    attach_annex_references_to_tree(main_tree)
    ...
    # ---------- 메타 ----------
    metadata = build_er_metadata(file_name, pdf_path)
    ...
    out_path = os.path.join(
        OUT_DIR,
        os.path.splitext(file_name)[0] + ".er.struct.json"
    )
...
def process_all():
    ...
    print(f"[ER parsing start] total={len(pdfs)}")
